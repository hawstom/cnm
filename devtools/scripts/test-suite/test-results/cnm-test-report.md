# CNM Test Suite Report

**Date:** 2025-38-04 06:38:07

**Status:** Running tests...

---

## Test Results

### TEST 1: Sta (Model)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "STA 53+00.00"
Actual:   "STA 53+00.00"
```

### TEST 1 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Sta"
XDATA composite-key: ("Sta" . "E0F")
XDATA auto-text: "STA 53+00.00"
```

### TEST 1 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Sta"
Reactor validation: SKIPPED (API unclear)
Handle: 289F6
```

### TEST 2: Dia (Model)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "18""
Actual:   "18""
```

### TEST 2 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Dia"
XDATA composite-key: ("Dia" . "E97")
XDATA auto-text: "18""
```

### TEST 2 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Dia"
Reactor validation: SKIPPED (API unclear)
Handle: 28C4C
```

### TEST 3: N (Model)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "N 20260.00"
Actual:   "N 20260.00"
```

### TEST 3 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "N"
XDATA composite-key: ("N" . "")
XDATA auto-text: "N 20260.00"
```

### TEST 3 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "N"
Reactor validation: SKIPPED (API unclear)
Handle: 28EA0
```

### TEST 4: LF (Model)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "%<\AcObjProp Object(%<\_ObjId 2499809461872>%).Length \f "%lu2%pr0%ct8[1]">% LF"
Actual:   "%<\AcObjProp Object(%<\_ObjId 2499809461872>%).Length \f "%lu2%pr0%ct8[1]">% LF"
```

### TEST 4 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "LF"
XDATA composite-key: ("LF" . "")
XDATA auto-text: "%<\AcObjProp Object(%<\_ObjId 2499809461872>%).Length \f "%lu2%pr0%ct8[1]">% LF"
```

### TEST 5: Manual (Model)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "MODEL MANUAL"
Actual:   "MODEL MANUAL"
```

### TEST 6: Name (Model)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "OAK ST"
Actual:   "OAK ST"
```

### TEST 6 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "AlName"
XDATA composite-key: ("AlName" . "E0F")
XDATA auto-text: "OAK ST"
```

### TEST 6 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "AlName"
Reactor validation: SKIPPED (API unclear)
Handle: 2959F
```

### TEST 7: SF (Model)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "%<\AcObjProp Object(%<\_ObjId 2499809461872>%).Area \f "%lu2%pr0%ct8[1]">% SF"
Actual:   "%<\AcObjProp Object(%<\_ObjId 2499809461872>%).Area \f "%lu2%pr0%ct8[1]">% SF"
```

### TEST 7 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "SF"
XDATA composite-key: ("SF" . "")
XDATA auto-text: "%<\AcObjProp Object(%<\_ObjId 2499809461872>%).Area \f "%lu2%pr0%ct8[1]">% SF"
```

### TEST 8: SY (Model)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "%<\AcObjProp Object(%<\_ObjId 2499809461872>%).Area \f "%lu2%pr0%ct8[0.11111111]">% SY"
Actual:   "%<\AcObjProp Object(%<\_ObjId 2499809461872>%).Area \f "%lu2%pr0%ct8[0.11111111]">% SY"
```

### TEST 8 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "SY"
XDATA composite-key: ("SY" . "")
XDATA auto-text: "%<\AcObjProp Object(%<\_ObjId 2499809461872>%).Area \f "%lu2%pr0%ct8[0.11111111]">% SY"
```

### TEST 9: Sta (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "STA 50+00.00"
Actual:   "STA 50+00.00"
```

### TEST 9 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Sta"
XDATA composite-key: ("Sta" . "E0F")
XDATA auto-text: "STA 50+00.00"
```

### TEST 9 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Sta"
Reactor validation: SKIPPED (API unclear)
Handle: 29CA1
```

### TEST 10: Off (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "20.00 LT"
Actual:   "20.00 LT"
```

### TEST 10 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Off"
XDATA composite-key: ("Off" . "E0F")
XDATA auto-text: "20.00 LT"
```

### TEST 10 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Off"
Reactor validation: SKIPPED (API unclear)
Handle: 29EF6
```

### TEST 11: StaOff (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "STA 51+20.00, 10.00 LT"
Actual:   "STA 51+20.00, 10.00 LT"
```

### TEST 11 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "StaOff"
XDATA composite-key: ("StaOff" . "E0F")
XDATA auto-text: "STA 51+20.00, 10.00 LT"
```

### TEST 11 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "StaOff"
Reactor validation: SKIPPED (API unclear)
Handle: 2A14B
```

### TEST 12: StaName (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "STA 56+40.00 OAK ST"
Actual:   "STA 56+40.00 OAK ST"
```

### TEST 12 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "StaName"
XDATA composite-key: ("StaName" . "E0F")
XDATA auto-text: "STA 56+40.00 OAK ST"
```

### TEST 12 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "StaName"
Reactor validation: SKIPPED (API unclear)
Handle: 2A3A0
```

### TEST 13: Dia (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "18""
Actual:   "18""
```

### TEST 13 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Dia"
XDATA composite-key: ("Dia" . "E97")
XDATA auto-text: "18""
```

### TEST 13 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Dia"
Reactor validation: SKIPPED (API unclear)
Handle: 2A5F5
```

### TEST 14: Slope (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "0.31%"
Actual:   "0.31%"
```

### TEST 14 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Slope"
XDATA composite-key: ("Slope" . "E97")
XDATA auto-text: "0.31%"
```

### TEST 14 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "Slope"
Reactor validation: SKIPPED (API unclear)
Handle: 2A849
```

### TEST 15: N (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "N 20260.00"
Actual:   "N 20260.00"
```

### TEST 15 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "N"
XDATA composite-key: ("N" . "")
XDATA auto-text: "N 20260.00"
```

### TEST 15 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "N"
Reactor validation: SKIPPED (API unclear)
Handle: 2AA9D
```

### TEST 16: E (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "E 10380.00"
Actual:   "E 10380.00"
```

### TEST 16 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "E"
XDATA composite-key: ("E" . "")
XDATA auto-text: "E 10380.00"
```

### TEST 16 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "E"
Reactor validation: SKIPPED (API unclear)
Handle: 2ACF2
```

### TEST 17: NE (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "N 20230.00, E 10440.00"
Actual:   "N 20230.00, E 10440.00"
```

### TEST 17 (XDATA)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "NE"
XDATA composite-key: ("NE" . "")
XDATA auto-text: "N 20230.00, E 10440.00"
```

### TEST 17 (Reactor)

**Status:** INFO

**Details:**
```
Tag: NOTETXT1
Expected auto-type: "NE"
Reactor validation: SKIPPED (API unclear)
Handle: 2AF47
```

### TEST 18: Manual (Paper)

**Status:** PASS

**Details:**
```
Tag: NOTETXT1
Expected: "PAPER MANUAL"
Actual:   "PAPER MANUAL"
```

### TEST H: Key Notes Table Generation

**Status:** PASS

**Details:**
```
✅ PASS: Key Notes Table generated
  Table blocks found: 24
  (Manual verification required for quantities)

  Expected:
    BOX-1: 325 LF (100+150+75)
    BOX-2: 2 EA (count)
    CIR-1: 200 LF
    CIR-2: 1 EA (count)
    DIA-1: 450 LF
    DIA-2: 3 EA (count)
    HEX-1: 800 LF
    HEX-2: 5 EA (count)

  Check: Visual inspection of table
  Check: cnm-test.not file created
```

### TEST H: .not File Creation

**Status:** PASS

**Details:**
```
.not file exists in project folder
```

### SETUP: Create Sheet List

**Status:** PASS

**Details:**
```
cnm-test.lst created with single entry: cnm-test
```

### TEST I: QT Table Generation

**Status:** PASS

**Details:**
```
✅ PASS: QT Table generated
  Text objects found: 78
  (Manual verification required for totals)

  Expected single-sheet totals:
    BOX-1: 325 LF
    BOX-2: 2 EA
    CIR-1: 200 LF
    CIR-2: 1 EA
    DIA-1: 450 LF
    DIA-2: 3 EA
    HEX-1: 800 LF
    HEX-2: 5 EA

  Check: Visual inspection of QT table
```

### TEST 39: Insertion Performance

**Status:** INFO

**Details:**
```
⏱ INSERTION PERFORMANCE: 20 bubbles with auto-text
  Total time: 35.42 seconds
  Average per bubble: 1771 ms
  Insertion rate: 0.6 bubbles/sec

  BASELINE: Establishes performance expectation
  - < 500ms/bubble: ✅ Excellent (typical)
  - 500-1000ms/bubble: ⚡ Acceptable
  - > 1000ms/bubble: ⚠️ Investigate bottlenecks

  Use this baseline for regression detection:
  - Re-run after code changes
  - Compare timing to detect performance regressions
  - Track trends over multiple test runs
```

### TEST 40: Reactor Performance

**Status:** INFO

**Details:**
```
⏱ REACTOR PERFORMANCE: 20 leader stretches
  Total time: 81.49 seconds
  Average per update: 4074 ms
  (Each stretch triggers VLR-OBJECT-REACTOR callback)

  PERFORMANCE ANALYSIS:
  ⚠️ SLOW: > 50ms per callback
  🔍 Check: Complex auto-text, multiple Civil 3D queries
  💡 Consider: Caching expensive operations

  BASELINE: Use for regression detection
  - Re-run after reactor code changes
  - Compare to detect performance impact
  - Track config optimization effects (Phase 3)

  DETAILED PROFILING:
  Check test-results/reactor-profiling.log for millisecond-level breakdown
  of XDATA writes, attribute updates, Civil3D queries, and formatting
```


---

## Summary

- **Total Tests:** 42

- **Model Space Tests:** 8

- **Paper Space Tests:** 10


**Note:** Review test entries above for PASS/FAIL status.

Review drawing annotations on C-ANNO-TEST-RESULTS layer for visual results.

### CLEANUP: Project files

**Status:** INFO

**Details:**
```
Removed files if present.
```

