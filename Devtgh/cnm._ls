...
(DEFUN
  ...
  (STRCAT PROJECT-FOLDER "\\cnm.ini")
)
(DEFUN
   HCNM-PROJECT-FOLDER-TO-LINK (PROJECT-FOLDER)
  (STRCAT PROJECT-FOLDER "\\cnmproj.txt")
)
    ...
    (PROJ
     ...
     (COND
       ...
       (PROJ
        ...
        (IF (NOT (FINDFILE (HCNM-INI-NAME PROJ)))
          (HCNM-INITIALIZE-PROJECT PROJ)
        )
        PROJ
       )
     )
...
(DEFUN
   HCNM-READ-ALL-configS (/ PROJECT-configS USER-configS)
  (SETQ
    *HCNM-CONFIG*
     (APPEND
       (INI_READSECTION
       ...
       ((MAPCAR
          '(LAMBDA (ENTRY)
             (list (HCNM-CONFIG-entry-var ENTRY)(HCNM-CONFIG-READ-USER (HCNM-CONFIG-entry-var ENTRY)))
           )
...
(DEFUN
  ...
  (COND
    ((HCNM-config-SCOPE-EQ VAR "User")(HCNM-config-WRITE-USER VAL))
    (T
...
(DEFUN
   hcnm-config-GETVAR (VAR / ADDTOLIST ADDTOPROJ DEFINE-configS DIR INI PROJROOT config VAL)
  (SETQ
    ADDTOLIST T
    ADDTOPROJ T
  ...
  (IF ADDTOLIST
    (SETQ
      *HCNM-CONFIG*
       (CONS (LIST config VAL) *HCNM-CONFIG*)
    )
  )
  (IF ADDTOPROJ
    (INI_WRITEENTRY
...
(DEFUN
  ...
  (COND
    ...
    (T
     ...
     (PRINC "[CNM]" F2)
     (SETQ F2 (CLOSE F2))|;TODO TEST AND REMOVE
